...
def robust_risk_budget_optimization(cov_matrix, risk_budget=None, max_iter=100000,
    ...
    bounds = Bounds(0, np.inf)  # 新约束：允许权重超过100%（允许杠杆）
...
def adjust_leverage(weights, bond_indices, total_target=1.0):
    """
    通过调整债券仓位使总权重等于目标值
    :param weights: 原始权重数组
    :param bond_indices: 债券资产的索引位置列表
    :param total_target: 目标总权重 (默认为1.0)
    :return: 调整后的权重
    """
    total_weight = np.sum(weights)
    leverage = total_weight - total_target  # 杠杆部分

    if leverage <= 0:
        return weights  # 无需调整

    # 计算债券总权重
    bond_total = np.sum(weights[bond_indices])

    if bond_total < leverage:
        # 债券=0
        new_weights = weights.copy()
        new_weights[bond_indices] = 0
        # 确保调整后总权重精确等于1
        final_total = np.sum(new_weights)
        if abs(final_total - 1.0) > 1e-6:
            new_weights *= total_target / final_total
        return new_weights

    else:
        # 在债券中按比例缩减杠杆部分
        reduction_ratio = leverage / bond_total
        new_weights = weights.copy()
        new_weights[bond_indices] = weights[bond_indices] * (1 - reduction_ratio)
...
def risk_parity_backtest(risk_budgets, start_date, end_date, target_volatility,
    ...
    ### 直接读excel
    master_df = pd.read_excel('prices投顾.xlsx')
    ###
    # 确保所有资产都有数据
    ...
    for i, date in enumerate(backtest_daily_returns.index):
        ...
        if date in monthly_rebalance_dates_real:
            ...
            # 识别债券资产索引
            bond_assets = [asset for asset, class_name in asset_class_mapping.items()
                           if class_name == '债券']
            bond_indices = [idx for idx, asset in enumerate(assets)
                            if asset in bond_assets]

            # 调整杠杆（使总权重=100%）
            adjusted_weights = adjust_leverage(
                new_weights,
            ...
            # 使用调整后的权重
            weights = pd.Series(adjusted_weights, index=assets)
            # print(weights)
