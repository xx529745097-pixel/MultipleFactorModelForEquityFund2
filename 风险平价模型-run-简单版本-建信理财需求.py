...
def risk_parity_backtest(risk_budgets, start_date, end_date, target_volatility,
                         ...
                         momentum_lookback=12, asset_selection=None, asset_class_mapping=None, file_name = None):
    """
    ...
    for i, date in enumerate(backtest_daily_returns.index):
        ...
        if date in monthly_rebalance_dates_real:
            ...
            print("=== 标准优化 ===")
            new_weights_result = robust_risk_budget_optimization(cov_matrix, risk_budget_list, verbose=False)
            print(date)
...
if __name__ == "__main__":
    ...
    RISK_BUDGETS_list = [RISK_BUDGETS2,RISK_BUDGETS3,RISK_BUDGETS4,RISK_BUDGETS5,RISK_BUDGETS6,RISK_BUDGETS7,RISK_BUDGETS8,RISK_BUDGETS9,RISK_BUDGETS10]
    # 新增：资产到大类的映射
    ASSET_CLASS_MAPPING = {
        # 'IC.CFE': '股票',  # A股
        # 'IF.CFE': '股票',  # A股
    ...
    for RISK_BUDGETS_temp in RISK_BUDGETS_list:
        pairs = [f"{k}={v}" for k, v in RISK_BUDGETS_temp.items()]
        combined = "_".join(pairs)
        valid_name = re.sub(r'[^\w\.=]', '', combined)
        file_name = f"{valid_name}.xlsx"

        # 执行回测（添加动量参数）
        net_value, daily_weights, monthly_weights = risk_parity_backtest(
            RISK_BUDGETS_temp, START_DATE, END_DATE, TARGET_VOLATILITY,
