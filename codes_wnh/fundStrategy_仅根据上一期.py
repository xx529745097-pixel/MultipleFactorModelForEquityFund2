...
def adjustDate(lkbk, freqi, date):
    ...
    adjustDate = adjustDate.loc[adjustDate['date'] > 20240601].reset_index(drop=True)
...
def dataPrepare(today):
    ...
    df_fund.replace(
        {"type": {"2001010101000000": "普通股票型", "2001010201000000": "偏股混合型基金", "2001010204000000": "灵活配置型基金"}},
        inplace=True)
    ...
    df_fund[["fmedate", "fmsdate", "sector_sdate", "sector_edate"]] = df_fund[
        ["fmedate", "fmsdate", "sector_sdate", "sector_edate"]].astype("int")
...
def loopMain(df_fund, df_aum, lkbki, freqi, today):
    ...
    for codei in fundId:
        ...
        for i, ini in enumerate(ads["date"]):
            ...
            if data_temp.query("(fmsdate<@sdate_temp)&(fmedate>=@edate_temp)").shape[0] > 0:  # 任何一个基金经理任职超过一年
                ...
                if temp.query("(sector_sdate<=@edate_temp)&(sector_edate>=@edate_temp)").shape[
                    0] > 0:  # 满足上一个条件的基金中，edate时是股票或偏股基金
    ...
    fundUniverse = pd.merge(fundUniverse, df_stktonav[['year', 'month', 'fcode', 'stktonav']], how='left',
                            on=['year', 'month', 'fcode'])
...
def ranking(fund, lkbk, stable=True):
    ...
    for date in date_list:
        ...
        WSS = w.w.wss([','.join(temp['fcode'])], "risk_jensen,risk_sharpe,risk_stock,risk_time",
                      "startDate=" + str(int(date) + lkbk) + ";endDate=" + str(
                          date) + ";period=2;returnType=1;riskFreeRate=1;index=000001.SH")
        ...
        result = result.loc[
            (~result['product_name'].str.contains('定开')) & (~result['product_name'].str.contains('持有'))].reset_index(
            drop=True)
...
def strategy(input,numoffund):
    ...
    portfolio = pd.read_excel('portfolio_lastperiod.xlsx')
    portfolio = portfolio[['date', 'fcode', 'score','status','nav']]
    # 缓冲池
    portfolio_new = output.loc[output['date'] == output['date'].max()].sort_values(by='score', ascending=False).iloc[:int(numoffund*3)].reset_index(drop=True).copy()
    portfolio_old = portfolio.copy()
