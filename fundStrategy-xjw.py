    ...
    # 周频监测时，仅当近一周超额回撤超过阈值触发临时调仓;仅当与上一次调仓不小于5个交易日才实行周度监测
        ...
        if excess_return > -excess_drawdown_threshold:
            ...
            if index == '000906.SH':
                ...
                mapping = wind.wind_getIndustriesMap('SW', 1, date=
                ann_date_temp).drop(['date'],axis=1)
                index_df = pd.merge(index_df, mapping, on=['stock_id'])
                benchmark_weights = index_df.groupby(['industry']).sum().reset_index()
                ...
                benchmark_weights = index_holding.groupby(['industry']).sum().reset_index()
            ...
            product_score_with_info = pd.merge(product_score_with_info, df_industry.drop('date', axis=1),
                                               on='product_id', how='left')

            # 通过 WDS 获取基准指数的行业权重
            index_df = wind.wind_getStockIndexComponentsWeight(
                'ZZ800',  # must be in "HS300", "ZZ100", "ZZ500", "ZZ800", "ZZ1000"
                wind.wind_getLastTradeDates([ann_date_temp])[0],  # datetime.date instance
                wind.wind_getLastTradeDates([ann_date_temp])[0]  # datetime.date instance
            )
            mapping = wind.wind_getIndustriesMap('SW', 1, date=ann_date_temp).drop(['date'], axis=1)
...
if __name__ == '__main__':
    ...
    for model_date in adj_calendar['model_date'].to_list():
        ...
        fstrat_getCC30ModelFinalProductList_changeable_diviation(model_date, model_freq=model_freq, shortlist_num=30, buffer_size=0,excess_drawdown_threshold=0.03,
                                                                 original_ind_deviation=1, original_deviation=1, temp_ind_deviation=0.01,temp_deviation=0.3, index='885001.WI', index_delay=1,
